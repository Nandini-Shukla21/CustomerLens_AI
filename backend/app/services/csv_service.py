...
class CSVService:
    """
    Service for validating, storing, summarizing,
    and managing uploaded CSV files.
    """
    ...
    async def save_file(self, file: UploadFile) -> Path:
        """Persist an uploaded CSV file to disk."""
    ...
    def load_dataframe(
        ...
        try:
            ...
            raise ValidationError(
                f"Unable to parse CSV file: {exc}"
            ) from exc
    ...
    async def handle_upload(
        ...
        """
        Validate, persist, parse, summarize,
        and register a CSV upload.
        """
    ...
    def _generate_dataset_id(
        ...
        existing_ids = {
            item["dataset_id"]
            for item in self.dataframe_manager.list_datasets()
        }
